...
class NotExistsItemException(BaseException):
    pass
...
class BasicMongodbAdapter:
    ...
    def _get_client(self):
        conn_string = self.db_url.format(username=self.db_username,
                                         password=self.db_password)
        return pymongo.MongoClient(conn_string, ssl_cert_reqs=ssl.CERT_NONE)
    ...
    def _get_table(self):
        return self._database[self.table_name]
    ...
    def _get_item_from_table(self, item_id):
        query = {'entity_id': item_id}
        return self._table.find_one(query)

    def list_all(self):
        documents = self._table.find({})
        return [self._instantiate_object(d) for d in documents]
    ...
    def _check_if_exists(self, item_id):
        ...
        if not item:
            raise NotExistsItemException(f"Item '{item_id}' não existe.")

    def save(self, json_data):
        cleaned_data = BasicMongodbAdapter._normalize_nodes(json_data)
        update_filter = {'entity_id': json_data.get('entity_id')}
        update_query = {'$set': cleaned_data}
        update_result = self._table.uptade_one(update_filter,
                                               update_query,
                                               upsert=True)
        return update_result.upserted_id

    def delete(self, item_id):
        query_delte = {'entity_id': item_id}
        delete_result = self._table.delete_one(query_delte)
    ...
    def _process_filter(self, keys, values):
        ...
        if len(filter_params) > 1:
            return key_name, self._process_filter_multiple(filter_params,
                                                           values)
