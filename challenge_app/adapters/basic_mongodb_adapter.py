...
class BasicMongodbAdapter:
    ...
    def _check_if_exists(self, item_id):
        ...
        if not item:
            raise NotExistsItemException(f'Item {item_id} does not exist.')
    ...
    def save(self, json_data):
        ...
        update_result = self._table.update_one(update_filter,
                                               update_query,
                                               upsert=True)
    ...
    def _process_filters(self, kwargs):
        ...
        for k, v in kwargs.items():
            key, value = self._process_filters(k, v)
            filters[key] = value
        return filters
