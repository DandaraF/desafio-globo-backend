...
class BasicPostgresqlAdapter:
    def __init__(self,
        ...
        self._logger = logger if logger else logging.get_logger()
    ...
    def _clean_list_empty_elements(arg):
        result = []
        for value in arg:
            clean_value = BasicPostgresqlAdapter._normalize_nodes(value)
            if clean_value:
    ...
    def _normalize_nodes(arg):
        cleaners = {set: BasicPostgresqlAdapter._clean_set_empty_elements,
                    list: BasicPostgresqlAdapter._clean_list_empty_elements,
                    dict: BasicPostgresqlAdapter._clean_dict_empty_elements}
    ...
    def save(self, json_data):
        cleaned_data = BasicPostgresqlAdapter._normalize_nodes(json_data)
        update_filter = {'_id': json_data.get('_id')}
        update_query = {'$set': cleaned_data}
        update_result = self._table.update_one(update_filter, update_query,
                                               upsert=True)
